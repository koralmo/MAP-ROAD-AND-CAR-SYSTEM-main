...
public class Junction extends utilities.Point implements RouteParts {
	...
	public  void checkIn(Vehicle vehicle) {
			...
			if(vehicle.getVehicleType().getAverageSpeed()<vehicle.getLastRoad().getMaxSpeed())
			{
				vehicle.getSupervisor().update(vehicle,vehicle);
				Mediator.setstate(vehicle,new HadReport());
			}
			if(vehicle.getstate() instanceof HadReport)
			{
				vehicle.getSupervisor().getReports().readFromFile(vehicle);
			}
	}
